...
export type AdminCommissionData = SharedData & {
  role: "admin";
  trend: AdminCommissionTrendPoint[];
  summary: CommissionSummary;
  previousSummary: CommissionSummary | null;
  analytics: AdminCommissionAnalytics;
  teams: { id: string; name: string }[];
  selectedTeamId?: string;
  showTeamFilter: boolean;
  exportHref: string;
  table: CommissionTablePage<CommissionRow>;
};

export type ManagerCommissionData = SharedData & {
  role: "manager";
  trend: CommissionTrendPoint[];
  summary: CommissionOnlySummary;
...
export type OrganizationCommissionData =
  | AdminCommissionData
  | ManagerCommissionData;

export type AgentCommissionData = SharedData & {
  role: "agent";
  trend: CommissionTrendPoint[];
  row: CommissionOnlyRow | null;
};
...
type ReportColumn = (typeof ADMIN_REPORT_COLUMNS)[number];
...
function PageHeader({ data, exportHref }: { data: SharedData; exportHref?: string }) {
  ...
  return (
    <header className={styles.pageHeader}>
      <div>
        <span className={styles.eyebrow}>{data.role === "admin" ? "Compensation" : "Commissions"}</span>
        <h1>Commissions</h1>
        <p>{data.role === "agent"
...
function isAdminTrendPoint(
  point: CommissionTrendPoint | AdminCommissionTrendPoint,
): point is AdminCommissionTrendPoint {
  return "baseSalaries" in point && "totalCompensation" in point;
}
...
function DashboardTrend({
  ...
  points: (CommissionTrendPoint | AdminCommissionTrendPoint)[];
  ...
  const series: { key: "commission" | "totalCompensation"; label: string; color: string }[] = [
  ...
  const trendValue = (
    point: CommissionTrendPoint | AdminCommissionTrendPoint,
    key: "commission" | "totalCompensation",
  ) => key === "commission"
    ? point.commission
    : "totalCompensation" in point
      ? point.totalCompensation
      : 0;
  const values = points.flatMap((point) => series.filter((item) => visible[item.key]).map((item) => trendValue(point, item.key)));
  ...
  const path = (key: "commission" | "totalCompensation") => points.map((point, pointIndex) => `${x(pointIndex)},${y(trendValue(point, key))}`).join(" ");
    ...
    <section className={`${styles.panel} ${styles.trendPanel}`} aria-labelledby="commission-trend-title">
      <header className={styles.panelHeader}>
        ...
        <div className={styles.legend} aria-label="Chart series">
          ...
          {salaryVisible && (!visible.commission || !visible.totalCompensation) ? <button className={styles.resetSeries} onClick={() => setVisible({ commission: true, totalCompensation: true })} type="button">Reset</button> : null}
        </div>
      </header>
      {points.length === 0 ? <EmptyState title="No commission history is available" detail="This is the first authoritative commission period in scope." /> : (
        <div className={styles.chartArea}>
          <svg
            aria-label={`${salaryVisible ? "Monthly commission and total compensation" : "Monthly commission"} trend. Use left and right arrow keys to inspect months.`}
          ...
          >
            ...
            {active ? series.map((item) => visible[item.key] ? <circle cx={x(index)} cy={y(trendValue(active, item.key))} fill="#fff" key={item.key} r={activeSeries === item.key ? 5 : 4} stroke={item.color} strokeWidth="2.5" /> : null) : null}
          ...
          {active ? <div aria-live="polite" className={styles.chartTooltip} style={{ "--tooltip-x": `${points.length <= 1 ? 50 : index / (points.length - 1) * 100}%` } as CSSProperties}><strong>{active.label}{active.estimated ? " (Estimated)" : ""}</strong><span><i style={{ background: "#2563eb" }} />Commission<b>{egp(active.commission)}</b></span>{salaryVisible && isAdminTrendPoint(active) ? <><span><i style={{ background: "#10b981" }} />Base salaries<b>{egp(active.baseSalaries)}</b></span><span><i style={{ background: "#10b981" }} />Total compensation<b>{egp(active.totalCompensation)}</b></span></> : null}<span>Closed deals<b>{number(active.closedDeals)}</b></span><span>Employees<b>{number(active.employees)}</b></span></div> : null}
        </div>
      )}
      <div className={styles.srOnly}><table><caption>Accessible commission trend data</caption><thead><tr><th>Month</th><th>Commission</th>{salaryVisible ? <><th>Base salaries</th><th>Total compensation</th></> : null}<th>Closed deals</th><th>Employees</th><th>Status</th></tr></thead><tbody>{points.map((point) => <tr key={point.key}><th>{point.label}</th><td>{egp(point.commission)}</td>{salaryVisible && isAdminTrendPoint(point) ? <><td>{egp(point.baseSalaries)}</td><td>{egp(point.totalCompensation)}</td></> : null}<td>{point.closedDeals}</td><td>{point.employees}</td><td>{point.estimated ? "Estimated" : "Final"}</td></tr>)}</tbody></table></div>
    </section>
  );
}
...
function RowPreview({
  ...
  row: CommissionOnlyRow | CommissionRow;
  onClose: () => void;
  salaryVisible: boolean;
}) {
...
function CommissionReport({ data, updateParams, pending }: { data: OrganizationCommissionData; updateParams: (changes: Record<string, string | null>) => void; pending: boolean }) {
  const reportColumns: readonly ReportColumn[] = data.role === "admin"
    ? ADMIN_REPORT_COLUMNS
    : COMMON_REPORT_COLUMNS;
  const [query, setQuery] = useState(data.table.query);
  const [columns, setColumns] = useState<Record<ReportColumn, boolean>>(() => Object.fromEntries(ADMIN_REPORT_COLUMNS.map((column) => [column, true])) as Record<ReportColumn, boolean>);
  const [selected, setSelected] = useState<CommissionOnlyRow | CommissionRow | null>(null);
  ...
  return (
    <section className={`${styles.panel} ${styles.reportPanel}`} aria-labelledby="commission-report-title">
      ...
      <div className={styles.tableToolbar}>
        ...
        <label>Sort by<select aria-label="Sort commission report" onChange={(event) => sort(event.target.value as CommissionSort)} value={data.table.sort}>{(["name", "closedDeals", "tier", "commission", ...(data.role === "admin" ? ["baseSalary" as const, "totalCompensation" as const] : [])] as CommissionSort[]).map((item) => <option key={item} value={item}>{sortLabel(item)}</option>)}</select></label>
      </div>
      <div className={styles.tableScroll} role="region" aria-label="Commission results" tabIndex={0}>
        <table><caption>Role-scoped monthly commissions</caption><thead><tr><th scope="col">{sortable("Real Name", "name")}</th>{visible("American Name") ? <th scope="col">American Name</th> : null}{visible("Email") ? <th scope="col">Email</th> : null}{visible("Team") ? <th scope="col">Team</th> : null}{visible("Closed Deals") ? <th className={styles.numeric} scope="col">{sortable("Closed Deals", "closedDeals")}</th> : null}{visible("Current Tier") ? <th scope="col">{sortable("Current Tier", "tier")}</th> : null}{visible("EGP per Deal") ? <th className={styles.numeric} scope="col">EGP per Deal</th> : null}{visible("Commission") ? <th className={styles.numeric} scope="col">{sortable("Commission", "commission")}</th> : null}{data.role === "admin" && visible("Base Salary") ? <th className={styles.numeric} scope="col">{sortable("Base Salary", "baseSalary")}</th> : null}{data.role === "admin" && visible("Total Compensation") ? <th className={styles.numeric} scope="col">{sortable("Total Compensation", "totalCompensation")}</th> : null}<th scope="col"><span className={styles.srOnly}>Actions</span></th></tr></thead><tbody>{data.table.rows.length === 0 ? <tr><td colSpan={reportColumns.filter((column) => columns[column]).length + 2}><EmptyState title="No employees match these controls" detail="Adjust the search or authorized team selection." /></td></tr> : data.table.rows.map((row) => <tr key={row.id} tabIndex={0}><th scope="row"><strong>{row.realName}</strong><small>{row.active ? "Active" : "Inactive · final closed-deal month"}</small></th>{visible("American Name") ? <td>{row.americanName || "Not provided"}</td> : null}{visible("Email") ? <td>{row.email || "Not provided"}</td> : null}{visible("Team") ? <td><span className={styles.teamBadge}>{row.team?.name || "Unassigned"}</span></td> : null}{visible("Closed Deals") ? <td className={styles.numeric}><strong>{row.closedDeals}</strong></td> : null}{visible("Current Tier") ? <td><span className={styles.tierBadge}>{row.tierLabel}</span></td> : null}{visible("EGP per Deal") ? <td className={styles.numeric}>{egp(row.ratePerDeal)}</td> : null}{visible("Commission") ? <td className={styles.numeric}><strong>{egp(row.commissionAmount)}</strong></td> : null}{data.role === "admin" && visible("Base Salary") && "baseSalary" in row ? <td className={styles.numeric}>{egp(row.baseSalary)}</td> : null}{data.role === "admin" && visible("Total Compensation") && "totalCompensation" in row ? <td className={styles.numeric}><strong>{egp(row.totalCompensation)}</strong></td> : null}<td><button aria-label={`Preview ${row.realName} commission`} className={styles.rowAction} onClick={() => setSelected(row)} type="button"><DashboardIcon name="arrowRight" /></button></td></tr>)}</tbody></table>
      </div>
      ...
      {selected ? <RowPreview onClose={() => setSelected(null)} row={selected} salaryVisible={data.role === "admin"} /> : null}
    </section>
  );
}
...
function OrganizationDashboard({ data }: { data: OrganizationCommissionData }) {
  ...
  const metricCards = data.role === "admin"
    ? [
        ...commonMetricCards,
  ...
  return <>
    ...
    <div className={styles.analyticsGrid}><DashboardTrend points={data.trend} salaryVisible={data.role === "admin"} /><TeamDistribution data={data} /><TierDistribution data={data} /></div>
    <CommissionReport data={data} pending={pending} updateParams={updateParams} />
  </>;
}

function PersonalProgress({ row }: { row: CommissionOnlyRow }) {
  const [open, setOpen] = useState(false);
...
function PersonalTrend({ points }: { points: CommissionTrendPoint[] }) {
  const [metric, setMetric] = useState<"commission" | "closedDeals">("commission");
  const [index, setIndex] = useState(Math.max(0, points.length - 1));
  const [inspecting, setInspecting] = useState(false);
  const width = 720;
  const height = 230;
...
function AgentDashboard({ data }: { data: AgentCommissionData }) {
  ...
  const cards = [
    ...
    { title: "Commission Earned", value: egp(row.commissionAmount), numeric: row.commissionAmount, previous: previous?.commission ?? null, support: "Tier earnings for this month", icon: "commissions" as const, tone: "violet" as const },
