...
export async function publishDialerImportBatch(input: {
  ...
  return getDb().transaction(async (tx) => {
    ...
    if (warnings.length > 0 && input.actor.role !== "admin") {
      throw new ImportConfirmationError(
        "Only an administrator can override import warnings.",
    ...
    await tx
      ...
      .set({
        ...
        confirmedAt: now,
        warningOverrideById: overrideReason ? input.actor.id : null,
        warningOverrideReason: overrideReason,
        warningOverrideAt: overrideReason ? now : null,
      })
      .where(eq(dialerImportBatches.id, batch.id));
    ...
    if (overrideReason) {
      await tx.insert(auditLogs).values({
        id: newId(),
        actorProfileId: input.actor.id,
        action: "dialer_import.warning_overridden",
        entityType: "dialer_import_batch",
        entityId: batch.id,
        metadata: {
      ...
      if (Array.isArray(duplicateImports) && duplicateImports.length > 0) {
        await tx.insert(auditLogs).values({
          id: newId(),
          actorProfileId: input.actor.id,
          action: "dialer_import.duplicate_overridden",
          entityType: "dialer_import_batch",
          entityId: batch.id,
          metadata: {
            reason: overrideReason,
    ...
    await tx.insert(auditLogs).values({
      ...
      metadata: {
        ...
        warningOverrideUsed: Boolean(overrideReason),
      },
    });
...
export async function getImportDetails(actor: Actor, batchId: string) {
  ...
  const [batch] = await getDb()
    .select({
      ...
      warningOverrideById: dialerImportBatches.warningOverrideById,
      warningOverrideReason: dialerImportBatches.warningOverrideReason,
      warningOverrideAt: dialerImportBatches.warningOverrideAt,
