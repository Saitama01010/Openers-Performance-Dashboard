...
export default async function CoachingRoomPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    ...
    <section className={styles.panel} aria-labelledby="past-sessions-title"><header className={styles.panelHeader}><div><h2 id="past-sessions-title">Past coaching sessions</h2><p>Newest grouped sessions first</p></div><span className={styles.muted}>{data.pagination.total} sessions</span></header><div className={styles.tableScroll}><table className={styles.table}><caption>Past coaching sessions and grouped participants</caption><thead><tr><th scope="col">Session Date</th><th scope="col">Coach</th><th scope="col">Category</th><th scope="col">Participants</th><th scope="col">Coached Agents</th><th scope="col">Coaching Note</th><th scope="col">Created</th></tr></thead><tbody>{data.rows.length === 0 ? <tr><td className={styles.empty} colSpan={7}>No coaching sessions match the authorized scope and filters.</td></tr> : data.rows.map((row) => <tr key={row.id}><td>{row.sessionDate}</td><th scope="row">{row.coachName}</th><td><span className={styles.badge}>{COACHING_CATEGORY_LABELS[row.category]}</span></td><td><ul className={styles.nameList}>{row.participants.map((participant) => <li key={participant.id}>{participant.name} · {participant.teamName}</li>)}</ul></td><td className={styles.numeric}>{row.participants.length}</td><td>{row.note ?? "—"}</td><td>{new Intl.DateTimeFormat("en-US", { dateStyle: "medium", timeStyle: "short", timeZone }).format(new Date(row.createdAt))}</td></tr>)}</tbody></table></div><footer className={styles.footer}><span>Showing {data.rows.length} of {data.pagination.total} sessions</span><nav aria-label="Coaching session pages" className={styles.pagination}>{page > 1 ? <Link href={href(params, page - 1)}>Previous</Link> : null}<span aria-current="page">{page}</span>{page < totalPages ? <Link href={href(params, page + 1)}>Next</Link> : null}</nav></footer></section>
  </>;
}
