    ...
    <>
      ...
      <div className={styles.tableWrap}>
        <table className={styles.table}>
          ...
          <tbody>{rows.map((user) => <tr key={user.id}><td><input aria-label={`Select ${user.name}`} checked={selected.has(user.id)} disabled={user.id === currentUserId || busy} onChange={() => setSelected((current) => toggleUserSelection(current, user.id))} type="checkbox" /></td><td><div className={styles.person}><span className={styles.avatar}>{initials(user.name)}</span><span><Link className={styles.personName} href={`/admin/users/${user.id}`}>{user.name}</Link>{user.id === currentUserId ? <span className={styles.you}>You</span> : null}<br /><span className={styles.sectionCopy}>{user.dialerAgentName ?? "No American name"}</span></span></div></td><td>{user.email ?? "—"}</td><td><select aria-label={`Role for ${user.name}`} className={styles.compactControl} disabled={busy || user.id === currentUserId} onChange={(event) => changeRole(user, event.target.value as UserRow["role"])} title={`Current role: ${roleLabel(user.role)}`} value={user.role}><option value="agent">Agent</option><option value="manager">Team Manager</option><option value="admin">Administrator</option></select></td><td>{user.role === "agent" || user.role === "manager" ? <InlineTeamSelect currentTeamId={user.teamId} currentTeamName={user.teamName} teams={activeTeams} userId={user.id} /> : "—"}</td><td><InlineShiftEditor shift={user.shift} userId={user.id} /></td><td><details className={styles.inlineDetails}><summary className={user.accountStatus === "active" ? styles.badgeSuccess : user.accountStatus === "deactivated" || user.accountStatus === "revoked" ? styles.badgeDanger : styles.badgeWarning}>● {statusLabel(user.accountStatus)}</summary><p className={styles.inlineDetailText}>{statusLabel(user.accountStatus)} account · Invitation: {statusLabel(user.invitationStatus)}</p></details></td><td><details className={styles.inlineDetails}><summary className={user.overrideCount ? styles.badgeInfo : styles.badge}>{user.overrideCount ? `${user.overrideCount} custom` : "None"}</summary><p className={styles.inlineDetailText}>{user.overrideCount ? user.overrideSummary.join(" · ") : "No explicit overrides. Effective access comes from the role."}</p></details></td><td><button className={styles.buttonGhost} onClick={() => openPreview(user.id)} type="button">View</button></td></tr>)}</tbody>
        </table>
      </div>
...
function UserPreview({ payload, currentUserId }: { payload: PreviewPayload; currentUserId: string }) {
  ...
  return <>
    <div className={styles.drawerPerson}><span className={styles.drawerAvatar}>{initials(user.name)}</span><div><strong>{user.name}</strong><p className={styles.sectionCopy}>{roleLabel(user.role)} · {user.email}</p><span className={user.accountStatus === "active" ? styles.badgeSuccess : styles.badgeDanger}>● {statusLabel(user.accountStatus)}</span></div></div>
